...
let _compression: typeof import("./compression-native.js") | null = null;
...
/**
 * Load compression native module
 * Returns null if not available or disabled
 */
export async function loadCompressionNative() {
  if (_compression) {
    return _compression;
  }
  ...
  try {
    const nativeDir = getNativeDir();
    const nativePath = path.join(nativeDir, "compression/compression.node");
    const mod = await import(nativePath);
    _compression = mod;
    return mod;
  } catch {
...
export function isNativeEnabled(cfg: NativeConfig, module: "fsSafe" | "archive" | "compression"): boolean {
